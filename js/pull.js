class PhysicsEngine {
  constructor() {
    ...
    this.navHeight = 50;
    this.canvas = document.querySelector(".pull");
  ...
  createBodies() {
    this.nav = Matter.Bodies.rectangle(
      this.canvasWidth / 2,
      this.navclosedPos,
      this.canvasWidth,
      this.navHeight,
      { isSensor: true, inertia: Infinity, mass: 0.1 }
    );
  ...
  createRope() {
    ...
    const rope = Matter.Composites.stack(
      ...
      0,
      (x, y) => {
        return Matter.Bodies.circle(400, y, 15, {
  ...
  createConstraints() {
    ...
    return [
      this.fixMenuToTop,
      this.fixMenuToBottom,
      fixRopeToMenu,
      this.fixMouseToHandle,
    ];
  }
...
class Nav {
  constructor() {
    ...
    this.closeNav = document.getElementById("close");

    this.closeNav.addEventListener("click", (e) => {
      this.release();
      this.close();
    });
  ...
  onResize() {
    ...
    this.ropeContainer.setAttribute(
      "viewport",
      `0 0 ${this.width} ${this.height}`
    );
    this.navOpenPos = (this.height / 10) * 6;
    this.physicsEngine.fixMenuToBottom.pointB.y = this.getCanvasY(
      this.navOpenPos
    );
  }
  ...
  move(e) {
    ...
    let x = e.clientX || e.touches[0].clientX;
    let y = e.clientY || e.touches[0].clientY;
    let navPos = this.getScreenY(
      this.physicsEngine.nav.position.y + this.physicsEngine.navHeight / 2
    );
    if (navPos >= 40 && !this.isOpen && !this.inTransition) {
      this.release();
      this.open();
    } else if (
      (navPos >= this.navOpenPos + 25 || navPos <= this.navOpenPos - 10) &&
      this.isOpen &&
      !this.inTransition
    ) {
      this.release();
  ...
  render() {
    ...
    let path = `M ${this.width / 2} ${this.getScreenY(
      this.physicsEngine.nav.position.y
    )}`;

    for (let body of this.physicsEngine.rope.bodies) {
      path += `L ${this.getScreenX(body.position.x)} ${this.getScreenY(
        body.position.y
      )}`;
    }

    let lastBody =
      this.physicsEngine.rope.bodies[this.physicsEngine.rope.bodies.length - 1];
    ...
    this.navElm.style.transform = `translate(${0}px, ${this.getScreenY(
      this.physicsEngine.nav.position.y + this.physicsEngine.navHeight / 2
    )}px)`;
  }
